...
test('2.53 counts any learner calendar booking with an address, including meetings',()=>{
...
test('2.53 never falls back to learner profile address after a booking is saved',()=>{
...
test('2.53 route choices come from dated calendar visits rather than profile-only sites',()=>{
...
test('2.53 mileage PDF follows the same calendar booking rules',()=>{
...
test('2.53 release metadata is aligned',()=>{
  assert.equal(pkg.version,'2.53.0');
  assert.equal(update.version,'2.53');
  assert.match(index,/milos-app-version" content="2\.53"/);
  assert.match(index,/milos-same-site-mileage-v251\.js\?v=2\.53/);
  assert.match(index,/milos-visit-address-v249\.js\?v=2\.53/);
  assert.match(index,/milos-mileage-pdf-v252\.js\?v=2\.53/);
  assert.match(sw,/milos-assessor-shell-v2\.53/);
});

...
test('current Milos release loads standardized UI last and offline',()=>{
  assert.match(index,/milos-standard-ui-v229\.css\?v=\d+\.\d+/);
  assert.match(index,/milos-standard-ui-v229\.js\?v=\d+\.\d+/);
...
test('Milos brand has a blue i and appears only in the top-left lockup',()=>{
  assert.match(css,/--std-brand:#2C85F7/);
  assert.match(js,/M<span class="app-brand-i">i<\/span>los/);
  assert.match(js,/Assessor assistant/);
  assert.match(js,/home\.querySelectorAll\('strong'\)\.forEach\(el=>el\.remove\(\)\)/);
  assert.match(js,/Tap me to get started/);
});

test('standard UI does not observe and rewrite the live app DOM',()=>{
  assert.doesNotMatch(js,/MutationObserver/);
  assert.match(js,/observer:false/);
});

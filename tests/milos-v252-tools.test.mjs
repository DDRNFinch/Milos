...
test('2.52 assessor details add college branding and place of work',()=>{
...
test('2.52 mileage export is PDF only and contains routes, leg miles and receipts',()=>{
...
test('2.52 release loads and caches all four isolated tools',()=>{
  for(const file of ['milos-profile-branding-v252.js','milos-full-calendar-v252.js','milos-route-selection-v252.js','milos-mileage-pdf-v252.js']){assert.match(index,new RegExp(file.replaceAll('.','\\.')+'\\?v=2\\.52'));assert.match(sw,new RegExp(file.replaceAll('.','\\.')));}
  assert.equal(pkg.version,'2.52.0');assert.equal(update.version,'2.52');assert.match(index,/milos-app-version" content="2\.52"/);assert.match(sw,/milos-assessor-shell-v2\.52/);
});

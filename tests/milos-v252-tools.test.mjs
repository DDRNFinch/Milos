...
test('current release still loads and caches all four 2.52 feature tools',()=>{
  for(const file of ['milos-profile-branding-v252.js','milos-full-calendar-v252.js','milos-route-selection-v252.js','milos-mileage-pdf-v252.js']){assert.match(index,new RegExp(file.replaceAll('.','\\.')+'\\?v=2\\.\d+'));assert.match(sw,new RegExp(file.replaceAll('.','\\.')));}
  assert.match(pkg.version,/^2\.\d+\.0$/);assert.match(update.version,/^2\.\d+$/);assert.match(index,/milos-app-version" content="2\.\d+"/);assert.match(sw,/milos-assessor-shell-v2\.\d+/);
});

...
const updater = fs.readFileSync(new URL("../assets/milos-updater-v236.js", import.meta.url), "utf8");
...
test("Milos has a visible non-blocking update action", () => {
  ...
  assert.doesNotMatch(updater, /milos-update-layer|waitCache/);
  assert.match(index, new RegExp(`milos-updater-v236\\.js\\?v=${version.replaceAll('.', '\\.')}`));
});
...
test("current Milos shell and manifest agree", () => {
  ...
  assert.match(sw, /milos-updater-v236\.js/);

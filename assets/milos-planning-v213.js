(function (global) {
  ...
  let returnDate = "";
  let reviewDefaultState = { userEdited: false, submitted: false };
  ...
  function readPlans() {
    try {
      ...
      return data.map((item) => ({
        ...
        sourceReviewId: clean(item && item.sourceReviewId, 120),
  ...
  function ensureStyle() {
    if (document.getElementById("milos-planning-v213-style")) return;
    const style = document.createElement("style");
    style.id = "milos-planning-v213-style";
    style.textContent = `
      .milos-planning-layer{position:fixed;inset:0;z-index:10040;background:linear-gradient(180deg,#f7fbff,#edf5ff);overflow:auto;-webkit-overflow-scrolling:touch;color:#17324d;pointer-events:auto}
      ...
      .milos-planning-icon{width:42px;height:42px;border:0;border-radius:50%;background:rgba(255,255,255,.86);box-shadow:0 5px 18px rgba(29,83,132,.09);font:inherit;font-size:24px;color:#245d91;cursor:pointer}
      ...
      .milos-planning-date{align-self:flex-start;width:24px;height:24px;border:0;border-radius:50%;background:transparent;color:#24445f;font:inherit;font-size:10px;font-weight:800;padding:0;cursor:pointer}.milos-planning-day.is-today .milos-planning-date{background:#2f8fef;color:#fff}
      .milos-plan-pill{width:100%;border:1px solid var(--plan-border);background:var(--plan-bg);color:var(--plan-text);border-radius:7px;padding:3px;font:inherit;font-size:8.5px;font-weight:800;line-height:1.15;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-align:left;cursor:pointer}.milos-plan-more{border:0;background:transparent;color:#4b7598;font:inherit;font-size:8.5px;font-weight:800;text-align:left;padding:1px 3px;cursor:pointer}
      ...
      .milos-planning-actions{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:15px}.milos-planning-actions .is-wide{grid-column:1/-1}.milos-planning-primary,.milos-planning-secondary,.milos-planning-danger{border:0;border-radius:13px;padding:12px;font:inherit;font-weight:800;cursor:pointer}.milos-planning-primary{background:#2f8fef;color:#fff}.milos-planning-secondary{background:#e8f2fb;color:#245d91}.milos-planning-danger{background:#fff0f0;color:#a23d3d}
  ...
  function saveBooking(form) {
    ...
    const id = clean(data.get("id"), 120);
    ...
    const record = {
      ...
      sourceReviewId: current ? current.sourceReviewId : "",
  ...
  function upsertNextReview(profileId, date, reviewId) {
    const cleanProfile = clean(profileId, 100);
    const cleanDate = safeDate(date);
    const cleanReview = clean(reviewId, 120);
    if (!cleanProfile || !cleanDate || !profileFor(cleanProfile)) return null;
    const items = readPlans();
    const id = cleanReview ? `next-review-${cleanReview}` : `next-review-${cleanProfile}-${cleanDate}`;
  ...
  function latestCompletedReview() {
    if (typeof C.getReviews !== "function") return null;
    const items = C.getReviews();
    if (!Array.isArray(items) || !items.length) return null;
    return items.slice().sort((a, b) => Number(b.completedAt || b.updatedAt || b.createdAt || 0) - Number(a.completedAt || a.updatedAt || a.createdAt || 0))[0] || null;
  }

  function syncLatestCompletedReview() {
    const review = latestCompletedReview();
    if (!review || !review.profileId || !review.nextReviewDate) return;
    upsertNextReview(review.profileId, review.nextReviewDate, review.id);
  }

  function resetReviewDefault() {
    reviewDefaultState = { userEdited: false, submitted: false };
  }

  function applyTenWeekDefault() {
    if (reviewDefaultState.userEdited || reviewDefaultState.submitted) return;
    const form = document.querySelector('form[data-form="review-targets"]');
    if (!form) return;
    const input = form.querySelector('input[name="nextReviewDate"]');
    if (!input || input.dataset.milosTenWeekDefault === "1" || !safeDate(input.value)) return;
    const tenWeekDate = addDays(input.value, -14);
    if (!tenWeekDate) return;
    input.value = tenWeekDate;
    input.dataset.milosTenWeekDefault = "1";
    const label = input.closest("label");
    const span = label && label.querySelector("span");
    if (span) span.textContent = "Next review date (booked in Planning)";
  }
  ...
  function syncMore() {
    ...
    const list = settingsButton.closest(".option-list") || panel.querySelector(".option-list");
    if (!list) return;
    if (!list.querySelector('[data-plan-action="open"]')) {
      const button = document.createElement("button");
      button.type = "button";
  ...
  function handlePlanClick(event) {
    const appAction = event.target && event.target.closest ? event.target.closest("[data-action]") : null;
    if (appAction && appAction.dataset.action === "start-review") resetReviewDefault();
    if (appAction && appAction.dataset.action === "review-complete") setTimeout(syncLatestCompletedReview, 0);
    ...
    const id = clean(target.dataset.id, 120);
    ...
    if (action === "start-review") { resetReviewDefault(); launchMilos("start-review", id); return; }
    if (action === "start-observation") { launchMilos("start-observation", id); return; }
    if (action === "add-learner") { launchMilos("new-learner", ""); }
  }

  document.addEventListener("click", handlePlanClick, false);

  document.addEventListener("input", (event) => {
    const input = event.target && event.target.matches ? event.target : null;
    if (input && input.matches('form[data-form="review-targets"] input[name="nextReviewDate"]') && event.isTrusted) {
      reviewDefaultState.userEdited = true;
    }
  }, true);

  document.addEventListener("submit", (event) => {
    const reviewForm = event.target && event.target.matches ? event.target : null;
    if (reviewForm && reviewForm.matches('form[data-form="review-targets"]')) reviewDefaultState.submitted = true;

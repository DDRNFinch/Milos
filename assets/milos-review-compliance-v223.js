...
function captureTargets(form){
  ...
  session.fields.nextReviewDate=next;
}
...
function patchRequired(input,labelText){
  ...
  if(label){label.classList.add("is-required");const span=label.querySelector(":scope > span");if(span&&labelText)span.textContent=labelText}
}
...
function patchMeeting(form){
  ...
  if(input&&!input.dataset.complianceHelp){
    input.dataset.complianceHelp="1";
    input.placeholder="Record what the employer contributed. If they could not contribute, record how and when they were invited.";
  }
}
...
function patchSupport(form){
  ...
  if(provider){const label=provider.closest("label");const span=label?.querySelector(":scope > span");if(span)span.textContent="Final review summary";provider.placeholder="Summarise the agreed position, key discussion points and what happens next."}
}
function patchTargets(form){
  ...
  if(!form.querySelector(".milos-compliance-target-note")){
    ...
    note.className="milos-form-note milos-compliance-target-note";
    note.textContent="Each agreed action needs a due date before the review can be completed.";
    const add=form.querySelector('[data-action="review-add-target"]');
    insertAfter(add,note);
  }
}

function stripMarkup(){
  const done=count();
  return `<div class="milos-compliance-strip" data-compliance-strip><span>Review check</span><strong>${done}/${ITEMS.length}</strong><small>required elements complete</small></div>`;
}
function patchStrip(form){
  if(form.querySelector("[data-compliance-strip]"))return;
  const heading=form.querySelector(".milos-wizard-heading");
  if(heading)insertAfter(heading,makeNode(stripMarkup()));
}
function panelMarkup(){
  return `<section class="milos-compliance-panel" aria-label="Review completion check"><div class="milos-compliance-heading"><span>Review completion check</span><strong>${count()}/${ITEMS.length}</strong></div><div class="milos-compliance-list">${ITEMS.map(([key,label])=>`<div class="milos-compliance-row${check(key)?" is-complete":""}" data-compliance-item="${key}"><i aria-hidden="true">${check(key)?"✓":""}</i><span>${label}</span><small>${check(key)?"Complete":"Required"}</small></div>`).join("")}</div><p class="milos-compliance-message" data-tone="info">${missing(false).length?`${missing(false).length} review element${missing(false).length===1?" is":"s are"} still missing.`:"Review content complete. Add the required signatures to finish."}</p></section>`;
}
function refreshFinal(){
  const button=document.getElementById("completeReviewButton");
  if(!button)return;
  if(!session.signaturesReady&&!button.disabled)session.signaturesReady=true;
  if(session.signaturesReady&&button.disabled)session.signaturesReady=false;
  let panel=document.querySelector(".milos-compliance-panel");
  const fresh=makeNode(panelMarkup());
  if(panel)panel.replaceWith(fresh);else{
    const summary=document.querySelector(".milos-review-summary");
    if(summary)insertAfter(summary,fresh);
  }
  ...
  if(contentMissing.length)button.title=`Complete ${contentMissing.length} required review element${contentMissing.length===1?"":"s"} first`;
...
function patch(){
  ...
  if(meeting)patchMeeting(meeting);
  if(progress)patchProgress(progress);
  if(support)patchSupport(support);
  if(targets)patchTargets(targets);
...
function captureSubmit(event){
  ...
  if(!kind.startsWith("review-"))return;
  try{
  ...
  }catch(error){event.preventDefault();event.stopImmediatePropagation();setMessage(error.message||"Complete the required review information.");const target=form.querySelector(":invalid")||form.querySelector("textarea, input, select");target?.focus();return}
}
function captureClick(event){
  ...
  if(action==="review-complete"){
    ...
    if(outstanding.length){event.preventDefault();event.stopImmediatePropagation();setMessage(`Complete: ${outstanding.map(item=>item[1]).join(", ")}.`);refreshFinal();return}
  }
}
...
function complianceRecord(record){
  const source=record||{};
  const extras={
    epaReadiness:text(session.fields.epaReadiness||source.epaReadiness),
    assessorJudgement:text(session.fields.assessorJudgement||source.assessorJudgement)
  };
  const rows=ITEMS.filter(([key])=>key!=="signatures").map(([key,label])=>({key,label,complete:key==="supportWellbeing"?!!(source.supportNeeds&&source.wellbeing):key==="targets"?Array.isArray(source.targets)&&source.targets.length>0&&source.targets.every(item=>item.title&&item.dueDate):key==="epaReadiness"?!!extras.epaReadiness:key==="assessorJudgement"?!!extras.assessorJudgement:!!source[key]}));
...
function addCompliancePage(doc,review){
  if(!review)return;
  doc.addPage();
  doc.setFont("helvetica","bold");doc.setFontSize(15);doc.text("Review compliance record",16,22);
  doc.setFont("helvetica","normal");doc.setFontSize(8);doc.text(`Milos ${VERSION} · completion evidence`,16,28);
  doc.setDrawColor(44,133,247);doc.line(16,33,194,33);
  let y=43;
  const paragraph=(label,value)=>{
    const lines=wrapLines(doc,value,174);
    if(y+lines.length*4.3+13>278){doc.addPage();y=22}
    doc.setFont("helvetica","bold");doc.setFontSize(7);doc.text(label.toUpperCase(),16,y);y+=5;
    doc.setFont("helvetica","normal");doc.setFontSize(8.2);doc.text(lines,16,y,{lineHeightFactor:1.3});y+=lines.length*4.3+8;
  };
  paragraph("EPA / assessment readiness",review.epaReadiness);
  paragraph("Assessor judgement",review.assessorJudgement);
  paragraph("Final review summary",review.providerComments);
  const compliance=review.reviewCompliance;
  if(compliance&&Array.isArray(compliance.items)){
    if(y+28>278){doc.addPage();y=22}
    doc.setFont("helvetica","bold");doc.setFontSize(7);doc.text("COMPLETION CHECK",16,y);y+=6;
...
async function reviewPdf(review){
  ...
  if(typeof Real!=="function")return originalReviewPdf(...args);
  function ComplianceJsPDF(){
    const doc=Reflect.construct(Real,Array.from(arguments));
    const save=doc.save.bind(doc);let added=false;
    doc.save=function(name){if(!added){addCompliancePage(doc,review);added=true}return save(name)};
    return doc;
  }
  ComplianceJsPDF.prototype=Real.prototype;
  try{ComplianceJsPDF.API=Real.API}catch(_){ }
  window.jspdf.jsPDF=ComplianceJsPDF;
  try{return await originalReviewPdf(...args)}finally{window.jspdf.jsPDF=Real}
}

(function (global) {
  ...
  function viewerCss() {
    return `
<style id="milosEvidenceViewerV272">
...
.mev-ready .notice{padding:8px 11px;margin:8px 0 10px;border-left-width:3px;font-size:12.5px;color:#516173;background:#f6f9fd}
.mev-ready .zipPick{margin:0 0 8px}
.mev-ready.mev-loaded .zipPick{display:none!important}
.mev-ready.mev-loaded .notice{display:none!important}
.mev-ready.mev-loaded .videoCol>.row:last-of-type{display:none!important}
...
@media(max-width:820px){
  ...
  .mev-ready .wrap{border:0;border-radius:0;box-shadow:none;padding:8px 10px 22px;max-width:none}
  .mev-ready h1{font-size:21px;margin-bottom:2px}
  .mev-ready .notice{font-size:12px;margin:6px 0 8px}
  .mev-ready .zipPick{position:relative;z-index:7;margin-bottom:8px;padding:12px 14px}
  ...
  .mev-ready .videoCol>.row:last-of-type{display:flex!important;margin:4px 4px 0!important;padding:0 2px}
  .mev-ready.mev-loaded .videoCol>.row:last-of-type{display:none!important}
...
@media(max-width:520px){
  ...
  function viewerScript() {
    return `
<script id="milosEvidenceViewerRuntimeV272">
(function(){
  "use strict";
  const VERSION="2.72";
  ...
  function restrictedLocal(){
    return location.protocol==="content:"||/Android/i.test(navigator.userAgent||"");
  }
  function requestZip(){
    if(zipReady())return true;
    if(restrictedLocal()){
  ...
  function markLoaded(){
    document.body.classList.add("mev-loaded");
    setQuietStatus("");
  }
  ...
  function render(){
    criteriaBox.innerHTML='<div class="mev-section-panel"><div class="mev-section-heading"><strong>Recorded evidence</strong><small>Choose a section, then tap a timestamp.</small></div><div class="mev-section-list">'+sections.map(function(section){
  ...
  function waitForZip(attempt){
    ...
    const loaded=String(player.src||"").startsWith("blob:")||/loaded from the zip|ready/i.test(status.textContent||"")||player.readyState>0;
    if(!loaded){
      if(attempt<40)setTimeout(function(){waitForZip(attempt+1)},100);
      return;
    }
  ...
  zipPicker.addEventListener("change",function(){
    if(zipReady())setQuietStatus("Loading evidence…");
    setTimeout(function(){waitForZip(0)},50);
  });

  player.addEventListener("loadedmetadata",function(){
    if(zipReady())markLoaded();
  });

  player.addEventListener("error",function(){
    setTimeout(function(){
      if(!zipReady()){
        setQuietStatus("Open the evidence ZIP once to load the recordings.");
      }
    },0);
  });

  const notice=document.querySelector(".notice");
  if(notice)notice.innerHTML="<strong>Private evidence viewer.</strong> Files stay on this device.";
  ...
  if(restrictedLocal())setQuietStatus("Open the evidence ZIP once to load the recordings.");

  global.MilosEvidenceViewerRuntime={version:VERSION,responsiveDesktop:true,stickyMobileVideo:true,desktopAutoRelative:true,phoneZipUnlock:true,directTimestampUnlock:true,quietBlockedState:true};
})();
</script>`;
  }

  function patchHtml(html) {
    if (!String(html || "").includes("Milos Evidence Viewer") || String(html || "").includes("milosEvidenceViewerV272")) return html;
    const evidenceJson = extractEvidenceJson(html);
    if (!evidenceJson) return html;
    let next = String(html);
    next = next.replace("</head>", `${viewerCss()}\n</head>`);
    next = next.replace("</body>", `<script type="application/json" id="milosEvidenceDataV272">${evidenceJson}</script>\n${viewerScript()}\n</body>`);
    return next;
  }
  ...
  global.MilosObservationBundle = Object.freeze(Object.assign({}, bundle, {
    makeZip,
    evidenceViewerV272: true,
    responsiveEvidenceViewer: true,
    desktopAutoRelativeMedia: true,
    phoneZipUnlock: true,
    directTimestampUnlock: true
  }));

  global.MilosEvidenceViewer272 = Object.freeze({
    version: VERSION,
